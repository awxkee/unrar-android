...
//    private final boolean encrypted;
    ...
    public boolean isPasswordProtected() throws IOException {
        ...
        long handle = -1;
        try {
            handle = openArchive0(rarPath, RAR_OM_LIST, flags, null);
            return isPasswordProtectedImpl(handle);
        } finally {
            if (handle != -1L) {
    ...
    public boolean validatePassword(@Nullable String password) throws IOException {
        int[] flags = new int[1];
        long handle = -1;
        try {
            handle = openArchive0(rarPath, RAR_OM_LIST, flags, password);
            return validatePasswordImpl(handle, password);
        } finally {
            if (handle != -1L) {
                closeArchive0(handle);
            }
    ...
    public boolean isEncrypted() throws IOException {
        int[] flags = new int[1];
        final long handle = openArchive0(rarPath, RAR_OM_LIST, flags, password);
        try {
     ...
     */
    public Iterable<RarEntry> getEntries(@NonNull final UnrarCallback callback) throws IOException {
        final long handle = openArchive0(rarPath, RAR_OM_LIST, null, password);
     ...
     */
    public void extract(@NonNull String entryName, @NonNull UnrarCallback callback) throws IOException {

        final long handle = openArchive0(rarPath, RAR_OM_EXTRACT, null, password);
        try {
            RarEntry header;
     ...
     * @throws IOException 打开rar异常
     */
    public InputStream getInputStream(@NonNull String entry) throws IOException {
     ...
     * @throws IOException
     */
    public void extractAll(@NonNull String destPath, @NonNull UnrarCallback callback) throws IOException {
        extractBatch(destPath, callback, entry -> true);
    }
     ...
     * @throws IOException
     */
    public void extractBatch(@NonNull String destPath, @NonNull UnrarCallback callback, ExtractFilter filter) throws IOException {
        final long handle = openArchive0(rarPath, RAR_OM_EXTRACT, null, password);
        try {
            RarEntry header;
            while ((header = readHeader0(handle, callback)) != null) {
     ...
     * @throws IOException
     */
    public void extractBatch2(@NonNull PasswordCallback passwordCallback, @NonNull ExtractFilter filter, @NonNull OnCreateOutCallback outCallback) throws IOException {
        final long handle = openArchive0(rarPath, RAR_OM_EXTRACT, null, password);
        try {
            RarEntry header;
    ...
    @Keep
    private static native long openArchive0(String rarName, int mode, @Nullable @Size(1) int[] flags, @Nullable String password) throws RarException;

    @Keep
    private static native boolean isPasswordProtectedImpl(long handle) throws RarException;

    @Keep
    private static native boolean validatePasswordImpl(long handle, @Nullable String password) throws RarException;
